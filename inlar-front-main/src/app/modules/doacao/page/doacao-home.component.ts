...
import { DoacaoService } from 'src/app/services/beneficiario/doacao.service';
...
export class doadorHomeComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.getDoacao(this.currentPage);
  }

  getDoacao(page: number): void {
    this.DoacaoService.getDoacao(page)
  ...
  deleteBeneficiario(beneficiario_id: number): void {
    if (beneficiario_id) {
      this.DoacaoService.deleteBeneficiario(beneficiario_id)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: () => {
            this.getDoacao(this.currentPage);  // Recarrega a lista após deletar
            this.messageService.add({
              severity: 'success',
              summary: 'Sucesso',
          ...
          error: (err: any) => {
            console.log(err);
            this.getDoacao(this.currentPage);  // Recarrega a lista, mesmo em caso de erro
            this.messageService.add({
              severity: 'error',
              summary: 'Erro',
  ...
  handleBeneficiarioAction(event: any): void {
    ...
    if (event) {
      ...
      this.ref.onClose.pipe(takeUntil(this.destroy$)).subscribe({
        next: () => this.getDoacao(this.currentPage),  // Recarrega a lista após fechar o diálogo
      });
    }
  }
