...
export class DoacaoItensFormComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        const doacaoitensData = this.config.data?.event;
    
        if (doacaoitensData) {
          this.isEditing = true;
          doacaoitensData.iddoacaoitens = doacaoitensData.id; 
          this.doacaoitensService.getDoacaoById(doacaoitensData.id) 
            .subscribe({
              next: (doacaoitens: GetDoacaoItensResponse) => {
                this.populateForm(doacaoitens); 
              },
              error: (err) => {
                this.handleErrorMessage('Erro ao buscar dados do item.');
              }
            });
        } else {
          this.isEditing = false;
          this.doacaoitensForm.reset();
        }
    }    
        ...
        this.doacaoitensService.createDoacao(formData)
          ...
          .subscribe({
            next: (response: GetDoacaoItensResponse) => {
              this.handleSuccessMessage('Itens registrados com sucesso!');
              this.ref.close();
            },
            error: (err) => {
              this.handleErrorMessage('Erro ao registrar itens!');
            }
        });
    }
    ...
    private editDoacaoitens(formData: any): void {
        ...
        this.doacaoitensService.updateDoacao(payload.id, payload)
          ...
          .subscribe({
            next: () => {
              this.handleSuccessMessage('Itens editados com sucesso!');
              this.ref.close();
            },
            error: (err) => {
              this.handleErrorMessage('Erro ao editar itens!');
            }
        });
    }
